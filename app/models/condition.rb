...
class Condition < ActiveRecord::Base
  ...
  def self.breakout_hash(conditions)
    view_data = Hash.new(0)
    view_data[:min] = (conditions.values.last)
    view_data[:max] = (conditions.values.first)
    
    if conditions.values.count != 0
      view_data[:avg] = conditions.values.sum/ conditions.values.count
    else 
      view_data[:avg] = 0
    end 
    view_data
    ...
    view_data = breakout_hash(conditions_in_range) 
	end 
